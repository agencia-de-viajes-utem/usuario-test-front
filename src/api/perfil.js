...
export const ObtenerInfoPerfil = async (token) => {
    ...
    try {
        ...
        const response = await fetch(`${import.meta.env.VITE_USUARIOS_BACKEND}/user/info`, requestOptions);
...
export const ActualizarPerfilUsuario = async (token, datosActualizados, navigate) => {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_USUARIOS_BACKEND}/user/update`, requestOptions);
...
export const UploadImageProfile = async (token, imageFile, filename, navigate) => {
    const uploadUrl = `${import.meta.env.VITE_USUARIOS_BACKEND}/user/upload-image`;
